...
> 특수 노트(Combo · Hold · BackToBack · Long Range · Missile)는 각각의 몬스터 캐릭터가 함께 등장합니다.
> **Normal**은 몬스터 없이 노트만 나옵니다.

| 노트 | 입력 방식 | 몬스터 | 특징 |
|------|-----------|--------|------|
| **Normal** | 단타 | — | 몬스터 없이 노트만 등장 |
| **Combo** | 양쪽 동시 단타 | Combo Golem | 좌우에서 동시에 노트 등장 |
| **Hold** | 키 유지 | 드래곤 | 홀드 길이를 채보 `length` 필드로 관리 |
...
| **경희대 아트코리아랩** 아트&테크 창업 지원 사업 성과 전시 | 2024-11 | 경희대학교 디지털콘텐츠학과 · [📰 동아일보](https://n.news.naver.com/mnews/article/020/0003674908?sid=101) |
...
| 카테고리 | 핵심 성과 |
|----------|----------|
| **인게임 시스템** | ms 기반 판정 시스템 재설계 · 특수 노트 4종(Hold, Rolling 등) 설계 및 구현 |
| **서버/경제** | Firebase 기반 Server-Authoritative 모델 구축 · BattlePass · Mission · 글로벌 리더보드 |
| **플랫폼/결제** | Steamworks SDK 소액결제 연동 · Gem 구매 시스템 · 오프라인 모드 대응 |
| **툴 개발** | **전용 채보 에디터(SOS NoteEditor) 메인테이너** · MIDI+DNN 자동 채보 생성 파이프라인 |
...
| **직접 구현 기능** | 총 61개 (인게임, 서버, 툴 통합) |
| **해결 버그/이슈** | 12건 (판정 로직, 데이터 동기화 등) |
| **총 커밋 수** | 587 커밋 (SoS-Refactoring + SOS_NoteEditor) |

👉 **[상세 포트폴리오 보기 (기술적 깊이 및 문제 해결 사례)](./PORTFOLIO.md)**